...
function display_list(key_word) {
  $("#suggestion-box")
    .text("")
    .hide();
  $.getJSON("https://web.archive.org/__wb/search/host?q=" + key_word, function(
    data
  ) {
    $("#suggestion-box")
      .text("")
      ...
      for (var i = 0; i < data.hosts.length; i++) {
        $("#suggestion-box").append(
          $("<li>").append(
            $("<a>")
              .attr("role", "button")
              .text(data.hosts[i].display_name)
              .click(event => {
...
function open_feedback_page() {
  var feedback_url =
    "https://chrome.google.com/webstore/detail/wayback-machine/fpnmgdkabkmnadcjpehmlllkndpkmiak/reviews?hl=en";
  chrome.tabs.create({ url: feedback_url });
}

function about_support() {
  window
    .open("about.html", "newwindow", "width=1200, height=900,left=0")
    .focus();
}
...
function settings() {
  window.open(
    "settings.html",
    "newwindow",
    "width=600, height=700,left=0,top=30"
  );
}
        ...
        $("#news_recommend_tr")
          ...
          .click(() => {
            if (
              ...
              event.show_context === undefined
            ) {
              chrome.tabs.create({
                url:
                  chrome.runtime.getURL("recommendations.html") + "?url=" + url
              });
            } else {
              chrome.system.display.getInfo(function(displayInfo) {
                ...
                chrome.windows.create({
                  url:
                    chrome.runtime.getURL("recommendations.html") +
                    "?url=" +
                    url,
                  width: width / 2,
                  height: height,
...
function show_wikibooks() {
  chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    ...
    if (url.match(/^https?:\/\/[\w\.]*wikipedia.org/)) {
      ...
      chrome.storage.sync.get(["wikibooks", "doi", "show_context"], function(
        event
      ) {
